...
void SQL_DBConnect()
{
	...
	char sQuery[256];
	FormatEx(sQuery, sizeof(sQuery), "CREATE TABLE IF NOT EXISTS ljroom (map TEXT PRIMARY KEY, x REAL, y REAL, z REAL, x1 REAL, y1 REAL);");
	g_DB.Query(SQL_TableCheck_Callback, sQuery);
}

public void SQL_TableCheck_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	if (results == null)
	{
...
void ImportLJRoomData()
{
	...
	while (!IsEndOfFile(file) && ReadFileLine(file, line, sizeof(line)))
	{
		...
		FormatEx(sQuery, sizeof(sQuery),
			"REPLACE INTO ljroom VALUES('%s', %s, %s, %s, %s, %s);",
